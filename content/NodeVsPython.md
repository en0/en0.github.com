...
Author: Ian Laird
Summary: A comparison between the execution speed of Python and NodeJs
...
NodeJS is a new language to me. We have begun to use it quite heavily at work
and it is clearly gaining popularity in the development communities. I have a
long list of things I dislike about NodeJS but we will save that for another
time.

What I want to share with you today is the result of some performance testing
that I did to see the execution speed of NodeJS as it compared to Python. To be
...
Blocking IO is a scenario that should showcase NodeJS's ability for effective
...
__Due to a bug in the loop counter, only 9 iterations where run on the NodeJS
implementation. The final run of the python version of this task was removed to
align the graph results.__

## Results

The results are not surprising. NodeJS had a much higher throughput as it was
...
the next task would be dispatched. This should affectively take away the NodeJS
event loop advantage._

## Results

The results were closer by percentage, but NodeJS still out performed Python by
...
NodeJS has already proven itself as a viable language for serious programing
...
NodeJs code are equally terrible. Deal with it.
...
[Test Source Code](http://github.com/en0/python-vs-nodejs)
